    ...
    if (dma_buffer) {
        ...
        if(cam_obj->jpeg_mode){
            ...
            if (offset_e >= 0) {
                ...
                static int no_eoi_count = 0;
                no_eoi_count++;
                // Only log every 20th NO-EOI to reduce spam
                if (no_eoi_count % 20 == 0) {
                    ESP_LOGW(TAG, "NO-EOI (count: %d)", no_eoi_count);
                }
                cam_give(dma_buffer);
                TickType_t elapsed = xTaskGetTickCount() - start;
                if (elapsed < timeout) {
                    return cam_take(timeout - elapsed);
                }
                return NULL;
            }
        ...
        if (timeout_count % 20 == 0) {
            ESP_LOGW(TAG, "Failed to get the frame on time! (count: %d)", timeout_count);
        }
    }
    return NULL;
}
